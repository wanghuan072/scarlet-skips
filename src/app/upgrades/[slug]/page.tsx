...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const item=getUpgrade(slug);if(!item)return {};return createMetadata({title:item.seo.title,description:item.seo.description,path:`/upgrades/${slug}`,image:item.image});}
